...
| `test_recording_support.py` | Unit | none | Coverage for the VCR sanitizer mechanisms in `src/vcr_sanitizers.py`: `_StreamedBodySerializationFix`, `_SafeBodyRedactor`/`_IdentityFieldRedactor`, `_GuidRedactor` (deterministic fixed-placeholder collapsing), the `tempauth` `QueryParamSanitizer` wiring, and a full-pipeline allowlist proof that no planted real-looking secret/PII value survives `VCR_SANITIZERS`. |
